...
| **D1** | **No "in progress" status color.** Three lead stages, two property statuses and three customer inquiry statuses were all *in progress* — not done, not failed, not inert, not a warning — and rendered `neutral`, a placeholder | **Closed 2026-07-14.** `info` family added to `DESIGN.md` (`#00668b` / `#bfe9ff`). A cyan works because **a status chip only ever appears next to other status chips** — it never has to compete with the brand blues | Done — swept 2026-07-16. ⚠️ Stitch screens for Kanban / leads table / lead detail were generated pre-`info` and must be regenerated |
| **P1** | *"Lead auto-assignment is undecided."* | **Closed — it was decided 2026-07-13 and this row never caught up.** `01-prd.md` **FR10.2** and its §18a decision log: *manual claim from a shared queue, no auto-assignment.* `03-database-schema.md` §10.1 #2 marks it **normative — "do not re-litigate."** The row even contradicted itself ("undecided… manual claim is specced") | Done — swept 2026-07-16 |
| **P2** | *"`01-prd.md` FR10.1 is stale — still promises configurable pipeline stages."* | **Closed — FR10.1 was corrected and this row was never swept.** FR10.1 now reads *"**Stages are a fixed set** — decided 2026-07-13; configurable stage names are explicitly out of scope for MVP."* `03-database-schema.md` §10.1 #1 already recorded the PRD as *"updated to match"* | Done — swept 2026-07-16 |
| **P3** | *"`04-api-spec.md` §14 is stale — implies a persisted report with an ID."* | **Closed — §14 was corrected and this row was never swept.** §14 now opens *"Reports are synchronous and stateless… The earlier `GET /admin/reports/{id}/export` shape is removed."* The real residue was §17's open-question checkbox, now closed | Done — swept 2026-07-16 |

### ⚠️ What P1/P2/P3 cost, and why they are kept here

**All three were real once.** Each was fixed in its owning doc — and **nobody swept this register.** So for three days the one file that is *"the only place a gap's state is recorded"* (§1.1) was **wrong about three of its own entries**, while `check_drift.py` reported `stale-gap: 0`.

An agent following the rules exactly — *"cite the gap ID, don't restate the list"* — would have read P2, believed the PRD was stale, and "fixed" a doc that was already correct. **The register being wrong is worse than a spec being wrong**, because the register is the thing specs are told to trust.

This is the same failure as §5A, inverted: §5A was a false claim propagating *outward* from a stale summary; this was a true fix that never propagated *inward* to the register. `check_drift.py`'s `stale-gap` check missed it because `CLOSED_GAPS` only listed `G1`/`G2`. **It now lists these too** (2026-07-16).
