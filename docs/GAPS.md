...
| File | Problem |
|---|---|
| `docs/stitch-prompts.md` | A **third** document competing with `11-stitch-design-prompts.md`. Its closing section says "paste this into Google Stitch" — but it contains **no design system at all**: no colors, no type, no tokens. Anyone following it produces off-brand screens. It is a good *functional* digest of `16-customer-spec/`; it is not a prompt set. **Decide: rename and re-scope it, fold it into doc 16, or delete it.** Per `OWNERSHIP.md` §6 it currently owns no concept. |
| `SPRINT0_PLAN.md` | Specifies **`psycopg2-binary`** (synchronous) against a `CLAUDE.md` rule that every I/O endpoint is `async` — SQLAlchemy's async engine needs **`asyncpg`**. Also lists **`@vitejs/plugin-tsx`**, which is not a real package (`@vitejs/plugin-react` handles TSX). Missing `pytest`, `ruff`, `black`, `httpx`, `pgvector`. **Fix before executing.** |
