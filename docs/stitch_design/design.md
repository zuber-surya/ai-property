...
|---|---|---|
| **Success** — done, live, approved, won | `success` `#026e4f` / `on-success` `#ffffff` | `success-container` `#c3f0da` / `on-success-container` `#00522f` |
| **Info** — **in progress.** Started, not finished. Nothing is wrong | `info` `#00668b` / `on-info` `#ffffff` | `info-container` `#bfe9ff` / `on-info-container` `#00344a` |
...
| **Error** — failed, rejected, invalid, urgent | `error` `#ba1a1a` / `on-error` `#ffffff` | `error-container` `#ffdad6` / `on-error-container` `#93000a` |
...
### Why `info` is a cyan, when the palette is already full of blues

`primary` is indigo, `secondary` is blue, `tertiary` is violet. Adding a *fifth* cool hue looks reckless — until you notice the constraint that actually applies:

> **A status chip only ever appears next to other status chips.**

`info` therefore has to be distinguishable from `success`, `warning`, `error` and `neutral` — **not** from the brand colors, which never render as status ("status is not brand"). That frees the cyan band. Pale cyan `#bfe9ff` against pale mint `#c3f0da` and pale amber `#ffddb0` is unambiguous at chip size; against indigo it never has to compete, because they never appear together.

Contrast: `info` on white **6.4 : 1**; `on-info-container` on `info-container` **10.3 : 1**.

### Canonical status mappings

Use these exactly. **Don't invent a color for a new state — map it onto one of the five.**

| Entity | State | Chip |
|---|---|---|
| **Property** | Draft — never been live | `neutral-container` |
| **Property** | Pending Approval — blocked on a human | `warning-container` |
| **Property** | Published — live | `success-container` |
| **Property** | On hold — paused, will return | **`info-container`** |
| **Property** | Rejected | `error-container` |
| **Property** | Sold — terminal | `inverse-surface` `#213145` / `inverse-on-surface` `#eaf1ff` |
| **Property** | Archived — terminal, inert | `neutral-container`, muted |
| **Lead** | New — nobody has touched it | `neutral-container` |
| **Lead** | Contacted · Site visit scheduled · Negotiation | **`info-container`** — all three |
| **Lead** | Closed — won | `success-container` |
| **Lead** | Closed — lost | `neutral-container`, muted. **Never `error`.** A lost deal is not a system failure, and the chip should not shout at the agent who lost it |
| **Customer inquiry** *(portal)* | Received | `neutral-container` |
| **Customer inquiry** | An agent is on it · Visit scheduled · In discussion | **`info-container`** |
| **Customer inquiry** | Completed | `success-container` |
| **Customer inquiry** | Closed | `neutral-container`, muted |
| **User** | Active / Invited / Inactive | `success` / `warning` / `neutral` container |

**The three in-progress lead stages share one color, deliberately.** They do not need three. The **text label** distinguishes them — status always carries one (see Accessibility) — and the Kanban column already encodes the stage positionally. Three near-identical blues would recreate the exact failure this ramp was built to fix.

**Why Sold is a dark chip, not a sixth hue:** Sold is *terminal*, not "extra good." A filled dark chip reads as **closed** against five pale ones and spends no new color.

### What this replaced

Before this ramp existed, Pending Approval and Published borrowed `secondary-container` `#2170e4` and `primary-container` `#4f46e5` — two blues **1.35 : 1** apart, effectively one color, on the two states an agent most needs to tell apart while scanning a list.

Then the ramp shipped without an in-progress color, and **three of the five lead stages had to render `neutral`** — a placeholder, not an answer. An agent could not tell "nobody has touched this" from "a site visit is booked". `info` closes that (gap **D1**).