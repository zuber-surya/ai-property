...
- **Follow-up reminders need wiring up — but every piece exists.** `lead_notes.follow_up_at` is stored, and nothing reads it yet. Both dependencies are real: the scheduler is `pg_cron` + a jobs worker (`02-architecture.md` §4.4 — the "no scheduler" claim was the false gap ~~G9a~~, `GAPS.md` §5A), and the notification store is `notifications` (`03-database-schema.md` §3.20, `type = 'follow_up_due'` is already an enum value). This is buildable work, not a blocker.
