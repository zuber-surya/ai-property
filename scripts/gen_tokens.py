...
def css_vars(design: dict) -> str:
    ...
    lines += [
        ...
        "}",
        "",
        "/* Status chips — pale fill + dark text. ALWAYS carry a text label. */",
        ...
        ".chip-status-draft    { composes: chip chip-neutral; }",
        ".chip-status-pending  { composes: chip chip-warning; }",
        ".chip-status-published{ composes: chip chip-success; }",
        ".chip-status-onhold   { composes: chip chip-info; }",
        ".chip-status-rejected { composes: chip chip-error; }",
        ".chip-status-sold     { composes: chip chip-terminal; }",
        "",
        "/* Lead stage. The three in-progress stages SHARE info (ADR-0018) — the",
        "   text label distinguishes them. Do not invent three blues. */",
        ".chip-stage-new         { composes: chip chip-neutral; }",
        ".chip-stage-contacted   { composes: chip chip-info; }",
        ".chip-stage-visit       { composes: chip chip-info; }",
        ".chip-stage-negotiation { composes: chip chip-info; }",
        ".chip-stage-won         { composes: chip chip-success; }",
        ".chip-stage-lost        { composes: chip chip-neutral; opacity: 0.7; }",
        "",
